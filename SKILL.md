...
description: "浏览器快照驱动的 E2E 测试：snapshot+Figma→用例→统一登录→单Worker串行执行→截图→报告。CRUD三连+页面级截图。"
---
...
- Write `meta.yaml`（平台地址+账号）和 `test-plan.yaml`
...
2. `browser snapshot` → save as snapshot_before
3. `browser screenshot` → save as `screenshots/{TC-ID}_before.png`
4. Execute action via browser act (use aria ref from snapshot)
5. Wait 1-2s
6. `browser snapshot` → save as snapshot_after
7. `browser screenshot` → save as `screenshots/{TC-ID}_after.png`
